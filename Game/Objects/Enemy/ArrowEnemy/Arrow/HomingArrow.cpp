...
void HomingArrow::Init(lwp::TransformEuler transform) {
	...
	model_.worldTF.scale = { 2.0f,2.0f,2.0f };
...
void HomingArrow::Attack() {
	...
	homingFrame_--;
	// 0よりも低くならないようにする
	homingFrame_ = std::min(max(homingFrame_, 0), kHomingEndFrame);
...
#pragma region 弾の角度
	...
	// X軸周りの角度(θx)
	model_.worldTF.rotation.x = std::atan2(-velocity_.y, velocityXZ);
#pragma endregion
}

void HomingArrow::ChangeHomingAccuracy() {
	// 自機との距離
	float distance = (model_.worldTF.translation - player_->GetWorldTransform()->translation).Length();
